(* ========================================================================= *)
(*                                                                           *)
(*       Implicational conversions and the MP_REWRITE tactic.                *)
(*                                                                           *)
...
let ALL_IMPCONV:imp_conv = fun _ _ -> ASSUME_IMP;;
...
let ASSUME_IMPCONV:imp_conv =
  fun ctx v t ->
    if t = T_ then failwith "ASSUME_IMPCONV"
    else
      if mem t (Context.terms_of ctx)
      then match v with Co -> DISCH T_ (ASSUME t) | Contra -> DISCH t TRUTH
      else failwith "ASSUME_IMPCONV";;
...
let PURE_REWRITE_IMPCONV ths =
  ...
  REDEPTH_IMPCONV (GENERAL_REWRITE_IMPCONV (DEEP_IMP_REWR_CONV ths)
    (*|| ctx_imp_conv_of_imp_conv (imp_conv_of_conv CONJ_CANON_CONV)*)
    || ASSUME_IMPCONV || EXISTS_IMPCONV);;
